...
import { AppShell, Breadcrumbs, Flex, SimpleGrid, Tooltip, rem, Divider } from '@mantine/core';
...
const Page = ({ params: { id } }: CurrentMoviePropsType) => {
  ...
  const movieGenres = data && data.genres.map(el => el.name).join();
  const date = data && parseISO(data.release_date);
  return (
    <AppShell navbar={{
      width: 280,
      breakpoint: 'sm',
          ...
          {data && (
            <div style={{ width: 800, minHeight: '100vh' }}>
              ...
              <Flex
                ...
                {data.poster_path ? (
                  <img style={{ width: 250 }} src={`https://image.tmdb.org/t/p/w500${data.poster_path}`}
                       alt={'Poster'} />
                ) : (
                  <div style={{
